...
appears in $S$ by writing $\langle f, t \rangle \in S$. We indicate
...
However, we take the entailment of the program $\Pi^C$ to be an in-order
stream $E^S$, where the order in which entailed fact instances appear in
$E^S$ depends on the order in which the fact instances in their
justifications appear in $S$. Given that that the instant of an entailed
...
\refdef{continuous datalog: instance justification}), $E^S$ will also
...
\[
E^T_i = \{ x \mid \Pi^{WC}_i \models x \land J^T \cup W^{S,r}_i \neq \varnothing \}
\]
where $J^T$ is a justification for the entailment of $x$ by $\Pi^{WC}_i$.
...
for every state $\Pi^{C,r}_i$ of every WC-Datalog program $\Pi^{C,r}$.
\end{definition}
...
over the stream of entailments $P$ at any given instant ${i \in \mathbb{T}}$
...
From this, we can see that each entailed fact instance $\langle f, e
\rangle$ in some $E^T_{i}$ has an instant $e$ that is associated with
the state $\Pi^{C,r}_e$ in which a justification for $\langle f, e
\rangle$ is first completed.

As such, the stream of entailed fact instances $P$ of
a $\Pi^{C,r}$ takes precisely the same form as the streams of facts already
