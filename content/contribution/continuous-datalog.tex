...
%For convenience, we will write $\Pi \models E$ to indicate that $E =
%cons(\Pi)$ is the set of all facts entailed by $\Pi$.
...
instances are as defined in \refdef{continuous datalog: fact
  instance}, and rules are of the same form as in conventional
Datalog programs.
...
The justification of entailments (as in \refdef{continuous datalog:
  justification}) is equally applicable to time-annotated Datalog
...
formally by \refdef{continuous datalog: instance justification}.
...
instances, as in \refdef{continuous datalog: fact instance}, we define
