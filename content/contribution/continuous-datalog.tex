...
$\Pi \models e$. For a a program $\Pi$ and a set of entailed facts
...
When considering a TA-Datalog program, in which there may
...
also satisfies $e$. The entailment of the program $\Pi^T$ is therefore:
...
\begin{align*}
  \Pi^T = \langle {} & \{ a \Leftarrow b \land c \}, \\
  & \{ \langle a, 1 \rangle, \langle a, 2 \rangle, \langle b, 1 \rangle, \langle b, 2 \rangle  \} \rangle
\end{align*}
...
This relationship is shown in \reffig{TD-D}.
...
into the following equivalent conventional Datalog program:
\[
  \Pi = \langle  \{ a \Leftarrow b \land c \}, \{ a, b \} \rangle
\]
...
Having defined the notion of time annotation in Datalog, we now extend
this concept to that of Continuous Datalog (henceforth C-Datalog).

\begin{definition}[Streams]
\labeldef{continuous datalog: stream}
A {\em stream} is a sequence of fact instances ordered
by their instants; in conjunction with the distinction between
...
A sliding window of range ${x \in \mathbb{N}}$ over a stream $S$ at
an instant ${i \in \mathbb{T}}$ is the \emph{set} of fact
