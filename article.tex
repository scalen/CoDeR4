...
the stream itself.  This is more similar to the aggregation functions of SQL,
...
$\Pi = \langle R, strip(F^0 \cup \mathop{\cup}\limits_{x=1}^n W^{S_j,r_j}_i \rangle$.
...
$\Pi^{C,r} = \langle R, F^0, \{ \langle S, r \rangle \} \rangle$ may be reconstructed from
...
\begin{gather}
  ...
  \labeleqn{continuous datalog: successive WC-Datalog program}
    \Pi^{C,x} = \langle R^\prime, F^{0\prime} \cup E^0, \{ W^{S^\prime,x} \} \cup \mathcal{W}^{P \setminus N} \rangle \models \langle E^{0\prime}, P^\prime, N^\prime \rangle
\end{gather}
...
\begin{nestedsection}{In the Semantic Web: Windowed Continuous RIF-Core}{continuous datalog: rif}
  ...
  ${\Pi^{C,r} = \langle R, F^0 \cup B^0, \{W^{S_1,r_1}, \dots, W^{S_n,r_n}\} \rangle}$.
...
\begin{nestedsection}{CoDeR Model for Continuous Deductive Reasoning}{model}
  ...
  It is composed of a stream-based semantic data model and a minimal algebra of stream-to-stream operators, the former for expressing the persistent fact instances $F^0$ and $E^0$, the streams $S_i$, $P$ and $N$ and the window ranges $r_i$ for $i \in (1,s)$ of each ${\Pi^{C,r} = \langle R,F^0 \cup B^0, \{ W^{S_x,r_x} \mid x \in (1,s) \} \rangle \models \langle E^0,P,N \rangle}$, the latter for expressing the persistent set of rules $R$, the built-in predicates of RIF-Core $B^0$ and enforcing the window range $r$.
  ...
  \begin{definition}[Empty Stream]\labeldef{CoDeR: empty stream}
    ...
    This stream may occur in specific expressions of any program, as described in \refsec{CoDeR: transformations}.
  \end{definition}
  ...
  \begin{nestedsection}{An Algebra for CoDeR}{model: algebra}
    ...
    However, three of these may be restricted when processing semantic data, as discussed in \refsec{background: semantic queries}, becoming:
    ...
    Because many of the operators' behaviours are defined in terms of bindings within graph instances for variables in the sequenced pattern associated with a received stream, the functions $vars(stream)$ and $binding(graph, variable)$ are defined:
    the former maps inter-operator streams to the sets of variables in their associated patterns;
    the latter maps graphs and variables from any inter-operator stream and associated pattern to the value bound to that variable in that graph.
    ...
    \begin{nestedsection}{Triple-Select}{CoDeR: triple-select}
      ...
      \begin{equation*}
        ...
        \left(
          \langle g, e, n \rangle \:\middle\vert\: \langle g, e, n \rangle \in X \land matches\left(g,\{triple\ .\}\right)
        \right)
      \end{equation*}
    \end{nestedsection}
    \begin{nestedsection}{External-Select}{CoDeR: external-select}
      ...
      As such, the produced stream will necessarily be the empty stream $\varnothing$ (as per \refeqn{CoDeR: empty result of external select over unbound variables} in \refsec{CoDeR: transformations}).

      More formally:
      % \begin{equation*}
      \begin{multline*}
        \sigma_{f(a_0,\dots,a_n)} \left( X \right) =
        \left(
          \langle g, e, n \rangle
        \:\middle\vert\:
          \langle g, e, n \rangle \in X \land \right. \\
          \begin{split}
            \forall x \in (0,n) v_x = \left\{
              \begin{split}
                & binding\left(g, a_x\right) && \text{, if } a_x \in vars(X) \\
                & a_x && \text{, otherwise}
              \end{split}\right. & \land \\
            f(v_{0},\dots,v_{n}) &
        \left.\right)
          \end{split}
      % \end{equation*}
    ...
    \begin{nestedsection}{Instance Implication}{CoDeR: instance implication}
      ...
      % \begin{equation*}
      \begin{multline*}
        \coloneq_{\{ a_0\ a_1\ a_2\ .\}} \left( X \right) =
        \left(
          \langle \{v_{0}\ v_{1}\ v_{2}\ .\}, e, n \rangle
        \:\middle\vert\:
          \langle g, e, n \rangle \in X \land \right. \\
            \forall x \in (0,2) v_x = \left\{
              \begin{split}
                & binding\left(g, a_x\right) && \text{, if } a_x \in vars(X) \\
                & a_x && \text{, otherwise}
              \end{split}
    ...
    \begin{nestedsection}{Stream Union}{CoDeR: stream union}
      ...
      \begin{equation*}
        ...
        \left(
          \langle g_i, e_i, n_i \rangle
        \:\middle\vert\:
            \langle g, e, n \rangle \in X \lor \langle g, e, n \rangle \in Y
        \right)
      \end{equation*}
    ...
    \begin{nestedsection}{SteM}{CoDeR: SteM}
      ...
      Since the persistent fact instances of $F^0$ are simply included as the CoDeR input stream $S_0$ and are annotated as being perpetually valid, as discussed in \refsec{CoDeR: data}, the applicable part of $F^0$ (and its entailment with respect to the rules $R$) will be perpetuated indefinitely in each SteM.
    ...
    In the remainder of this section: $a$ and $b$ are triple patterns; $g$ is a sequenced pattern; $p$ and $q$ are non-ground propositions using RIF-Core built-in predicates; $A$, $B$ and $C$ are CoDeR streams; and $X$, $Y$ and $Z$ are inter-operator streams.

    Equations \ref{eqn:CoDeR: identity of graph-to-triples under succession} to \ref{eqn:CoDeR: identity of streams under union} enumerate the unconditional \emph{identity} transformations of the operators described in \refsec{CoDeR: graph-to-triples}.
    \begin{align}
      \labeleqn{CoDeR: identity of graph-to-triples under succession}
      {}_g\pi_t\ {}_g\pi_t A &= {}_g\pi_t A \\
      \labeleqn{CoDeR: identity of instance implication under succession}
      \coloneq_g\ \coloneq_g X &=\ \coloneq_g X \\
      \labeleqn{CoDeR: identity of streams under union}
      A \cup A &= A
    \end{align}
    In addition, there are five identities that are conditional on the nature of the propositions $p$ and $q$ and the triple patterns $a$ and $b$.
    The first of these are presented in \refeqn{CoDeR: identity of triple select under succession,CoDeR: identity of triple select under succession to instance implication}, where applying $\sigma_{a}$ to a stream associated with the singleton pattern ${\{b\ .\}}$ (as produced by $\sigma_{b}$ and $\coloneq_{\{b\ .\}}$) in the case where $b$ subsumes $a$ (or ${a \subseteq b}$) produces a stream that is identical to the original.
    The same is true for \refeqn{CoDeR: identity of external select under succession}, only for propositions $p$ and $q$ where $q$ subsumes $p$ (or ${p \subset q}$ e.g. ${p = greater{-}than(?x,5)}$ and ${q = greater{-}than(?x,4)}$).
    Furthermore, in \refeqn{CoDeR: identity of graph-to-triples under succession to instance implication,CoDeR: identity of graph-to-triples under succession to triple-select}, applying ${{}_g\pi_t}$ to a stream associated with a singleton pattern (as produced by $\sigma_{triple}$ and $\coloneq_{\{a\ .\}}$) produces a stream that is identical to the original.
    \begin{align}
      \labeleqn{CoDeR: identity of triple select under succession}
      \sigma_b\ \sigma_a X &= \sigma_a X && \text{where } a \subseteq b \\
      \labeleqn{CoDeR: identity of triple select under succession to instance implication}
      \sigma_b \coloneq_{\{a\ .\}} X &=\ \coloneq_{\{a\ .\}} X && \text{where } a \subseteq b \\
      \labeleqn{CoDeR: identity of external select under succession}
      \sigma_q\ \sigma_p X &= \sigma_p X && \text{where } p \subseteq q \\
      \labeleqn{CoDeR: identity of graph-to-triples under succession to instance implication}
      {}_g\pi_t \sigma_{a} X &=\ \sigma_{a} X \\
      \labeleqn{CoDeR: identity of graph-to-triples under succession to instance implication}
      {}_g\pi_t \coloneq_{\{a\ .\}} X &=\ \coloneq_{\{a\ .\}} X
    \end{align}

  \refeqn{CoDeR: commutativity of triple select,CoDeR: commutativity of external select,CoDeR: commutativity of union} enumerate the \emph{commutative} transformations of the operators described in \refsec{CoDeR: operator detail}.
  These are the transformations that involve the re-ordering of unary operators of the same kind within a plan, as well as the swapping of inputs to binary operators, to produce another for the same sequenced pattern.
  \begin{align}
    \labeleqn{CoDeR: commutativity of triple select}
    \sigma_a\ \sigma_b X &= \sigma_b\ \sigma_a X \\
    \labeleqn{CoDeR: commutativity of external select}
    \sigma_p\ \sigma_q X &= \sigma_q\ \sigma_p X \\
    \labeleqn{CoDeR: commutativity of union}
    A \cup B &= B \cup A
  \end{align}
  It should be noted that the SteM operator $\rstreamjoin$ is \emph{not} commutative, unlike the symmetric window-join.
  However the definition of the window-join in terms of the commutative stream union and complementary SteMs in \refeqn{window-join-from-union-of-SteMs} does affirm the fact that the window-join \emph{is} commutative.

  \refeqn{CoDeR: associativity of union,CoDeR: associativity of SteM} enumerate the associative transformations of the operators described in \refsec{CoDeR: operator detail}.
  These are the transformations that involve the re-ordering of binary operators of the same kind within a plan to produce another for the same sequenced pattern.
  \begin{align}
    \labeleqn{CoDeR: associativity of union}
    A \cup \left( B \cup C \right) &= \left( A \cup B \right) \cup C \\
    \labeleqn{CoDeR: associativity of SteM}
    X \rstreamjoin \left( Y \rstreamjoin Z \right) &= \left( X \rstreamjoin Y \right) \rstreamjoin Z
  \end{align}   

  \refeqn{CoDeR: distributivity of selects,CoDeR: distributivity of triple select over union,CoDeR: distributivity of external select over union,CoDeR: distributivity of instance implication over union,CoDeR: distributivity of graph-to-triples over union} enumerate the distributive transformations of the operators described in \refsec{CoDeR: operator detail}.
  These are the transformations that involve the re-ordering of operators of different kinds within a plan to produce another for the same sequenced pattern.
  \begin{align}
    \labeleqn{CoDeR: distributivity of selects}
    \sigma_a\ \sigma_p X &= \sigma_p\ \sigma_a X \\
    \labeleqn{CoDeR: distributivity of graph-to-triples over union}
    {}_g\pi_t \left( A \cup B \right) &= \left( {}_g\pi_t A \right) \cup \left( {}_g\pi_t B \right) \\
    \labeleqn{CoDeR: distributivity of triple select over union}
    \sigma_a \left( X \cup Y \right) &= \left( \sigma_a X \right) \cup \left( \sigma_a Y \right) \\
    \labeleqn{CoDeR: distributivity of external select over union}
    \sigma_p \left( X \cup Y \right) &= \left( \sigma_p X \right) \cup \left( \sigma_p Y \right) \\
    \labeleqn{CoDeR: distributivity of instance implication over union}
    \coloneq_{g} \left( A \cup B \right) &= \left( \coloneq_{g} A \right) \cup \left( \coloneq_{g} B \right)
  \end{align}
  In addition, there are three conditional transformations, enumerated in \refeqn{CoDeR: distributivity of external select over SteM to left,CoDeR: distributivity of external select over SteM to right,CoDeR: distributivity of external select over SteM to both}, that distribute $\sigma_{external}$ over $\rstreamjoin$ and are analogous to those distributing selection $\sigma$ over join $\Join$ in the relational algebra.
  Consider the non-ground propositions
  \begin{align*}
    p &= greater{-}than(?1, 5) & q &= less{-}than(?3, 5)
  \end{align*}
  and associations between inter-operator streams and sequenced patterns
  \begin{align*}
    X &\mapsto \{?1\ pred1\ ?2\ .\} & Y &\mapsto \{?2\ pred2\ ?3\ .\} & Z &\mapsto \{?1\ pred3\ obj\ .\}
  \end{align*}
  The conditional transformations are thus: 
  % In the first case, non-ground proposition $p$ requires a set of variables ${\{?1\}}$, wholly subsumed by those present in the pattern associated with $X$ but not subsumed by the pattern associated with $Y$, to be bound.
  % As such, $\sigma_{p}$ may be distributed over $\rstreamjoin$ to $X$ but not to $Y$ (due to \refeqn{CoDeR: empty result of external select over unbound variables,CoDeR: identity of empty stream under SteM right} making ${\left( \sigma_p X \right) \rstreamjoin \left( \sigma_p Y \right) = \varnothing}$).
  \begin{equation}
    \labeleqn{CoDeR: distributivity of external select over SteM to left}
    \sigma_p \left( X \rstreamjoin Y \right) = \left( \sigma_p X \right) \rstreamjoin Y
  \end{equation}
  % In the second case, the non-ground proposition $q$ requires a set of variables ${\{?3\}}$, wholly subsumed in the pattern associated with $Y$ but not that associated with $X$, to be bound.
  % As such, $\sigma_{q}$ may be distributed over $\rstreamjoin$ to $Y$ but not to $X$ (due to \refeqn{CoDeR: empty result of external select over unbound variables,CoDeR: identity of empty stream under SteM left}).
  \begin{equation}
    \labeleqn{CoDeR: distributivity of external select over SteM to right}
    \sigma_q \left( X \rstreamjoin Y \right) = X \rstreamjoin \left( \sigma_q Y \right)
  \end{equation}
  % In the final case, the set of variables required by $p$ to be bound is wholly subsumed in the patterns associated with both $X$ and $Z$.
  % As such, $\sigma_{p}$ may be distributed over $\rstreamjoin$ to both $X$ and $Z$.
  \begin{equation}
    \labeleqn{CoDeR: distributivity of external select over SteM to both}
    \sigma_p \left( X \rstreamjoin Z \right) = \left( \sigma_p X \right) \rstreamjoin \left( \sigma_p Z \right)
  \end{equation}
  % It should be noted that, in the final case, $\sigma_p$ may alternatively be distributed to only one or the other of streams $X$ and $Y$ as in \refeqn{CoDeR: distributivity of external select over SteM to left,CoDeR: distributivity of external select over SteM to right};
  % this is because filtering certain bindings for, for example, ${?1}$ from $X$ will prevent $\rstreamjoin$ from finding matches for those bindings for ${?1}$ if they should occur in $Z$, and vice versa.

  Finally, the \emph{empty stream}, as per \refdef{CoDeR: empty stream}, may be created in a number of ways and creates a set of special identities, the latter enumerated in \refeqn{CoDeR: identity of empty stream under graph-to-triples,CoDeR: identity of empty stream under triple select,CoDeR: identity of empty stream under external select,CoDeR: identity of empty stream under instance implication,CoDeR: identity of empty stream under SteM right,CoDeR: identity of empty stream under SteM left,CoDeR: identity of stream under union with empty stream}.
  \begin{align}
    \labeleqn{CoDeR: identity of empty stream under graph-to-triples}
    {}_g\pi_t \varnothing &= \varnothing \\
    \labeleqn{CoDeR: identity of empty stream under triple select}
    \sigma_a \varnothing &= \varnothing \\
    \labeleqn{CoDeR: identity of empty stream under external select}
    \sigma_p \varnothing &= \varnothing \\
    \labeleqn{CoDeR: identity of empty stream under instance implication}
    \coloneq_g \varnothing &= \varnothing \\
    \labeleqn{CoDeR: identity of empty stream under SteM right}
    X \rstreamjoin \varnothing &= \varnothing \\
    \labeleqn{CoDeR: identity of empty stream under SteM left}
    \varnothing \rstreamjoin X &= \varnothing \\
    \labeleqn{CoDeR: identity of stream under union with empty stream}
    \varnothing \cup A &= A
  \end{align}
  The ways in which the empty stream is created all stem from the possibility that a successor $\sigma_\ast$ operator might filter the stream that it receives based on a pattern that will never match any graph instance produced by the operator that it succeeds.
  \refeqn{CoDeR: empty result of triple select under succession,CoDeR: empty result of triple select under succession from instance implication,CoDeR: empty result of external select under succession,CoDeR: empty result of external select over unbound variables} provide an example of each possible case where this occurs.
  \begin{align}
    \labeleqn{CoDeR: empty result of triple select under succession}
    \text{Conflicting patterns:} && \sigma_{?1\ pred1\ ?2}\ \sigma_{?1\ pred2\ ?2} X &= \varnothing \\
    \labeleqn{CoDeR: empty result of triple select under succession from instance implication}
    \text{Conflicting patterns:} && \sigma_{?1\ pred1\ ?2} \coloneq_{\{?1\ pred2\ ?2\ .\}} X &= \varnothing \\
    \labeleqn{CoDeR: empty result of external select under succession}
    \text{Mutually exclusive propositions:} && \sigma_{greater{-}than(?1,5)}\ \sigma_{less{-}than(?1,5)} X &= \varnothing \\
    \labeleqn{CoDeR: empty result of external select over unbound variables}
    \text{Unbound variables in proposition:} && \sigma_{greater{-}than(?3,5)}\ \sigma_{?1\ pred1\ ?2} X &= \varnothing
  \end{align}
  \refeqn{CoDeR: empty result of external select over unbound variables} expresses the extension of rule safety in RIF-Core relating to the required binding elsewhere in a pattern of any variables that are necessary arguments to a built-in predicate or function in that pattern.
  The example given assumes that both arguments of the concisely-named predicate ${greater{-}than()}$ must be bound, as they must in the actual built-in predicate of RIF-Core that it emulates:
  \[
    {<}http://www.w3.org/2007/rif{-}builtin{-}predicate\#numeric{-}greater{-}than{>}
  \]
  However, there are a few cases where specific arguments of specific predicates need not be bound \citep{w3crifcoreBuiltins}; the transformation in \refeqn{CoDeR: empty result of external select over unbound variables} does \emph{not} apply in the cases where unbound variables appear as those specific arguments.
\end{nestedsection}

\begin{nestedsection}{CoDeR Streams Notation}{model: stream notation}
  In compliment to the set of operators of the CoDeR algebra, we propose an extended notation that allows the accurate description a system built according to the CoDeR model, taking into account the iterative nature of inference and the uniqueness of streams within the model.
  This notation is needed to express two of the fundamental characteristics of the CoDeR data model, those being the intention of performing inference and the uniqueness of streams within the data model.
  ...
  This theoretically necessitates the union of the output stream with the input stream ``upstream'' of the remaining operators of any processing plan in the form of a \emph{working stream}, as per \refdef{CoDeR: working stream} and as shown in \reffig{CoDeR: re-entrant stream}.
  ...
  \begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{CoDeR-streams}
    \caption[Streams in CoDeR System]{An abstract CoDeR-based system, demonstrating the purview of each class of stream: Input and Output streams to receive data from and publish data to external sources; Inter-operator streams facilitate operator nesting/pipelines; and the re-entrant Working Stream facilitates iterative processing.}
    \labelfig{CoDeR: re-entrant stream}
  \end{figure}
  This feedback of the output stream into the \emph{working stream} may cause cause the definition of the output stream of a system to be circular, thus impossible to express purely in terms of the operators of the algebra described in \refsec{CoDeR: algebra}.
  This problem of inexpressibility also applies to the sharing of streams between potentially many sinks and sources:
  when relying solely on the nested-operator notation of the relational algebra, the same stream must be defined multiple times throughout a plan if it occurs in multiple sub-plans, though in reality that stream need only occur once in the modelled system.

  As such, the complete expression of the processing required for inference over WC-RIF-Core programs necessitates that streams may be named and that, once named, those streams may be referenced by those names within the definitions of other streams.
  Further, it must be possible to provide multiple contributing definitions for each named stream, this being an expression of \emph{stream union} derived from the manner in which the same predicate may have multiple disjunctive intensions within a Datalog program.
  ...
  These requirements are met in CoDeR using the notation shown in \refeqn{CoDeR: stream assignment}, an example of which for expressing the plan for some $\Pi^{C,r}$ is given in \refeqn{CoDeR: stream assignment example}.
  \begin{align}
    [stream\ name] &= [stream\ definition] \labeleqn{CoDeR: stream assignment} \\
    \begin{split}
      working\_stream &= {<}\text{http://example.org/test\_stream}{>}\\
      S_{\{?s\ ?p\ ?o\ .\}} &= {}_g\pi_t\ working\_stream \\
      S_{\{?s\ inside\ ?o\ .\}} &= \sigma_{?s\ inside\ ?o}\ S_{\{?s\ ?p\ ?o\ .\}} \\
      S_{\{?1\ inside\ ?2\ .\ ?2\ inside\ ?3\ .\}} &=\ S_{\{?1\ inside\ ?2\ .\}} \rstreamjoin S_{\{?2\ inside\ ?3\ .\}} \\
      S_{\{?1\ inside\ ?2\ .\}} &=\ \coloneq_{\{?1\ inside\ ?2\ .\}}\ S_{\{?1\ inside\ ?3\ .\ ?3\ inside\ ?2\ .\}} \\
      & \vdots \\
      inference\_stream &= S_{\{?s\ inside\ ?p\ .\}} \cup \dots \cup \dots \\
      working\_stream &= inference\_stream
    \end{split} \labeleqn{CoDeR: stream assignment example}
  \end{align}
  The first line of \refeqn{CoDeR: stream assignment example} demonstrates the definition of the plan's input stream by its IRI, and its being named ${working\_stream}$.
  The third through fifth lines demonstrate the dual definition of a single inter-operator named stream using variant graph patterns, in this case expressing the transitive property of the example predicate ${inside}$ via the definition of of an inter-operator named stream (using \refeqn{window-join-from-union-of-SteMs,CoDeR: identity of streams under union}).
  The penultimate line demonstrates the definition of the plan's inference stream, this being the \emph{stream union} of all streams produced by processing the separate rules of $R$.
  The last line asserts that the inference stream contributes through \emph{stream union} with the input stream to the previously named ${working\_stream}$, thereby making the plan iterative.

  In the plan presented in \refeqn{CoDeR: stream assignment example}, the stream defined on the fourth line is defined once and used once.
  As such, the stream definition on the fifth line may be rewritten, replacing the stream reference $S_{\{?1\ inside\ ?3\ .\ ?3\ inside\ ?2\ .\}}$ with its definition ${\left( S_{\{?1\ inside\ ?2\ .\}} \rstreamjoin S_{\{?2\ inside\ ?3\ .\}} \right)}$ from the fourth line, renaming variables as appropriate.
  This results in the plan presented in \refeqn{CoDeR: stream assignment example rewritten}.
  \begin{equation}
    \begin{split}
      working\_stream &= {<}\text{http://example.org/test\_stream}{>}\\
      S_{\{?s\ ?p\ ?o\ .\}} &= {}_g\pi_t\ working\_stream \\
      S_{\{?s\ inside\ ?o\ .\}} &= \sigma_{?s\ inside\ ?o}\ S_{\{?s\ ?p\ ?o\ .\}} \\
      S_{\{?1\ inside\ ?3\ .\}} &=\ \coloneq_{\{?1\ inside\ ?3\ .\}}\ \left( S_{\{?1\ inside\ ?2\ .\}} \rstreamjoin S_{\{?2\ inside\ ?3\ .\}} \right) \\
      & \vdots \\
      output\_stream &= S_{\{?s\ inside\ ?p\ .\}} \cup \dots \cup \dots \\
      working\_stream &= output\_stream
    \end{split} \labeleqn{CoDeR: stream assignment example rewritten}
  \end{equation}
  This method of rewriting may also be used to isolate a part of the plan as a single algebraic definition, to which the transformations presented in \refsec{CoDeR: transformations} may be applied.
  This rewriting process may then be carried out in reverse, assigning names to streams produced by sub-definitions, then replacing those sub-definitions with references to the corresponding named streams in the original definition.

  The example plans presented in \refeqn{CoDeR: stream assignment example,CoDeR: stream assignment example rewritten} also demonstrate that iteration within a plan follows naturally from the uniqueness of inter-operator streams by their associated sequenced pattern, discussed in \refsec{CoDeR: data}:
  in each case, the second definition of the stream $S_{\{?1\ inside\ ?3\ .\}}$ is recursive.
  The generalised case is of a rule whose head is a singleton graph pattern ${\{h\ .\}}$ that is a variant of the pattern of some triple-select operator $\sigma_{h}$;
  both the triple-select and instance implication contribute to the same stream, resulting in an iterative feedback of entailed graph instances into the plan that bypasses the \emph{working stream}, as shown in \reffig{CoDeR: unique stream feedback}.
  \begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{CoDeR-unique-stream-feedback}
    \caption[Iteration by Uniqueness of Streams in CoDeR]{An abstract CoDeR-based system, demonstrating the natural iterative feedback caused by the uniqueness of streams in the model.}
    \labelfig{CoDeR: unique stream feedback}
  \end{figure}
  To support the correctness of such behaviour, it can be shown that any such iterative feedback may be derived algebraically from the definition and reference of the working stream in any plan that expresses it precisely as in \refdef{CoDeR: working stream}.
  %
  % In order to derive this proof, the selection of triple instances matching the pattern $h$ from the \emph{working stream}, i.e. ${\sigma_{h}\ {}_{g}\pi_{t}\ working\_stream}$, must be rewritten (as already described) until all instance implication operators of the plan are present in a single algebraic expression.
  % This rewriting is demonstrated in \refeqn{CoDeR: named stream rewriting}, where:
  % \begin{itemize}
  %   \item
  %     $S_{g}$ is any inter-operator stream the precise nature of whose associated sequenced pattern is not relevant to the proof;
  %   \item
  %     $f$ is any sequenced pattern that is strictly incompatible with $h$ as per the transformation expressed in \refeqn{CoDeR: empty result of triple select under succession from instance implication};
  %   \item
  %     `$\dots$' is the remainder of the inter-operator streams that contribute to the CoDeR output stream, and whose associated sequenced pattern is neither $h$ nor incompatible with it.
  % \end{itemize}
  % \begin{equation} \labeleqn{CoDeR: named stream rewriting}
  %   \begin{split}
  %     \sigma_{h}\ {}_g\pi_t\ working\_stream &= \sigma_{h}\ {}_g\pi_t\ \left( input\_stream \cup output\_stream \right) \\
  %     &= \sigma_{h}\ {}_g\pi_t\ \left( input\_stream \cup \left( S_{\{h\ .\}} \cup \dots \cup S_{\{f\ .\}} \right) \right) \\
  %     &= \sigma_{h}\ {}_g\pi_t\ \left( input\_stream \cup \left( \left( \coloneq_{\{h\ .\}} S_{g} \right) \cup \dots \cup \left( \coloneq_{\{f\ .\}}\ S_{g} \right) \right) \right)
  %   \end{split}
  % \end{equation}
  % The proof may then be derived by showing that the filtering of the entire working stream by $h$ is equal to the union of the entailed stream matching pattern ${\{h\ .\}}$ with the remainder of the working stream filtered by $h$, less those entailed streams that match patterns $f$ that are incompatible with $h$.
  % This result is presented in \refeqn{CoDeR: transformations for closer iteration}, and may be proven by applying the following transformations to the right hand side:
  % \ref{eqn:CoDeR: associativity of union} and \ref{eqn:CoDeR: commutativity of union}%;
  % to re-arrange the union;
  % \ref{eqn:CoDeR: distributivity of graph-to-triples over union} and \ref{eqn:CoDeR: distributivity of triple select over union}%;
  % to distribute the selection by $h$;
  % \ref{eqn:CoDeR: identity of graph-to-triples under succession to instance implication}%;
  % to simplify each selection;
  % \ref{eqn:CoDeR: identity of triple select under succession to instance implication}%;
  % to eliminate selection over the entailed stream matching ${\{h\ .\}}$;
  % and \ref{eqn:CoDeR: empty result of triple select under succession from instance implication} and \ref{eqn:CoDeR: identity of stream under union with empty stream}%.
  % to eliminate from consideration the entailed streams matching ${\{f\ .\}}$.
  % \begin{equation} \labeleqn{CoDeR: transformations for closer iteration}
  %   \left( \begin{split}
  %     \left( \coloneq_{\{h\ .\}}\ S_{g} \right) \cup \\
  %     \left( \sigma_{h}\ {}_g\pi_t\ \left( input\_stream \cup \dots \right) \right)
  %   \end{split} \right)
  %   &
  %     =\ \sigma_{h}\ {}_g\pi_t\ \left( \begin{split}
  %       input\_stream \cup \\
  %       \left( \left( \coloneq_{\{h\ .\}} S_{g} \right) \cup \dots \cup \left( \coloneq_{\{f\ .\}} S_{g} \right) \right)
  %     \end{split} \right)
  % \end{equation}
