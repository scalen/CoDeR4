...
${- : \mathbb{T} \times \mathbb{N} \rightarrow \mathbb{T}}$.
...
$\mathcal{I}^T \subseteq \hat{H}^T$ satisfying every
clause (rule in $R$ and fact instance in $F^T$) in $\Pi^T$ also satisfies
$\langle e, t_e \rangle$, written $\Pi^T \models \langle e, t_e \rangle$.
As with facts in positive Datalog, we say that an interpretation
$\mathcal{I}^T$ satisfies a fact instance $\langle f, t \rangle \in F^T$
iff $\langle f, t \rangle \in \mathcal{I}^T$.  We say the interpretation
$\mathcal{I}^T$ satisfies a rule
$(r = \alpha_0 \Leftarrow \alpha_1 \land \ldots \land \alpha_n) \in R$ iff,
given $\{\langle a_1, t_1 \rangle, \dots, \langle a_n, t_n \rangle\} \subseteq \mathcal{I}^T$
such that $\{a_1, \dots, a_n\}$ satisfies the body of $r$,
$\langle a_0, t_0 \rangle \in \mathcal{I}^T$ (where $t_0 = max(t_1,\dots,t_n)$,
see \refdef{continuous datalog: instance justification}) such that
$\{a_0, \dots, a_n\}$ satisfies $r$.  The {\em entailment of the TA-Datalog
...
justification of that fact instance $J^T$, the instant $t_e$ at which
$\langle e, t_e \rangle$ becomes true is, intuitively, taken to be the
latest instant at which any of the fact instances in the justification
became true:
