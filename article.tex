...
\[ cons^T(\Pi^T) = \{ \langle e, t_e \rangle \in \hat{H}^T \mid
\Pi^T \models \langle e, t_e \rangle \} \]
...
became true:
\[ t_e = max(\{ t \mid \langle j, t \rangle \in J^T \}) \]
...
interval is defined as the set of numbers ${\{ x \mid i < x \leqslant i \}}$,
...
drawn from more than one source, and so $S$ may be a union of
the mappings represented by $n$ in-order streams $S_0,\dots,S_n$ without altering
...
\{ \langle S_0, r_0 \rangle,\dots,\langle S_n, r_n \rangle \} \rangle$
...
$r_x \in \mathbb{N}$, for all $x \in [0,n]$; we refer to this as the {\em instantaneous state} of
the WC-Datalog program, written
$\Pi^{C,r}_i = \langle R, F^0 \cup W^{S_0,r_0}_i \cup \dots \cup W^{S_n,r_n}_i \rangle$.
...
$\Pi = \langle R, strip(F^0 \cup W^{S_0,r_0}_i \cup \dots \cup W^{S_n,r_n}_i) \rangle$.
...
least partially contained in $W^{S_0,r_0}_i \cup \dots \cup W^{S_n,r_n}_i$.
{\nobreak We define $E^T_i$ as:
\[
E^T_i = \{ x \mid \Pi^{C,r}_i \models x \land J^T \cap \left( W^{S_0,r_0}_i \cup \dots \cup W^{S_n,r_n}_i \right) \neq \varnothing \}
\]
...
The decomposition of streams into more sparse streams without loss of
information allows us to define the state of a valid window
$W^{P \setminus N}$ as a union of the states of sliding windows over
each of the singleton streams $S_{\langle f, t \rangle} =
(\langle f, t \rangle \mapsto t^\prime)$, where
$S(\langle f, t \rangle) = t^\prime$.  The range of the sliding
window over each $S_{\langle f, t \rangle}$ is then the difference between
the instant at which $\langle f, t \rangle$ occurs in $P$ and that
at which it occurs in $N$:
\[
  W^{P \setminus N}_i = \{ \langle f, t \rangle \mid \langle f, t \rangle \in W^{S_{\langle f, t \rangle},N(\langle f, t \rangle) - P(\langle f, t \rangle)}_i\}
\]
