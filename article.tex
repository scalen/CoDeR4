...
of an entailment and its time-annotated justification, as per
...
Based on this definition of streams, we go on to define the notion of
sliding windows over streams, as well as states of those windows at
specific instants.  We do not consider count-based window functions, as
these serve a semantic purpose at high specificity, e.g. \emph{retain
...
which comprises a finite set of rules $R$, a finite set of persistent/static
fact instances $F^0$ (all of which have the instant $0$, meaning that they
cannot appear in any state of any window, by
\refdef{continuous datalog: window} and Axioms~\ref{axiom:continuous datalog: window range leq 0} and
...
drawn from more than one source, and so $S$ may be a union of $n$ in-order
streams $S_1,\dots,S_n$ without altering the semantics presented here.
...
in-order stream, $E^S$ expresses the times at which each entailed fact
instance was first justified by $\Pi^C$.
...
$\Pi^{C, r} = \langle R, F^0, \{ W^{S_1, r_1},\dots, W^{S_n, r_n} \rangle\} \rangle$
(where $r = max(r_1,\dots,r_n)$) to be a tuple with components $R$ and $F^0$ as defined above for a
C-Datalog program, but with each part of $S = S_1 \cup \dots \cup S_n$
associated with the range $r_x \in \mathbb{N}$ of a sliding window over
$S_x$, for $x \in [1,n]$. This specifies that the fact instances
...
\{ W^{S_1, r_1},\dots, W^{S_n, r_n} \} \rangle$
...
$r_x \in \mathbb{N}$, for all $x \in [1,n]$; we refer to this as the {\em instantaneous state} of
the WC-Datalog program, written
$\Pi^{C,r}_i = \langle R, F^0 \cup \mathop{\cup}\limits_{x=1}^n W^{S_j,r_j}_i \rangle$.
...
least partially contained in $\mathop{\cup}\limits_{x=1}^n W^{S_j,r_j}_i$.
{\nobreak We define $E^T_i$ as:
\[
E^T_i = \left\{ e \,\middle\vert\, \Pi^{C,r}_i \models e \land \left( J^T \cap \bigcup_{x=1}^n W^{S_j,r_j}_i \neq \varnothing \right) \right\}
\]
...
windows of sufficient range {\nobreak over its entailed streams $P$ and $N$ at
$i$:
...
case of $n$ windows of varying range, with $x \geqslant max(r_0,\dots,r_n)$.
...
sparse streams without loss of information allows us to define the
state of a valid window $W^{P \setminus N}$ as a union of the states
of sliding windows over each of the singleton streams
$(\langle f, t \rangle \mapsto t)$, the union of which is equal to $P$,
where $P(\langle f, t \rangle) = t$ means that $P$ maps
$\langle f, t \rangle$ to $t$.  The range of the sliding
window over each singleton stream is the difference between
the instant at which $\langle f, t \rangle$ occurs in $P$ ($t$, as $P$
is in-order) and that at which it occurs in $N$:
\[
  W^{P \setminus N}_i = \left\{ \langle f, t \rangle \,\middle\vert\, \begin{split}
    P(\langle f, t \rangle) = t \land \\
    \langle f, t \rangle \in W^{\left( \langle f, t \rangle \mapsto t \right),N(\langle f, t \rangle) - t}_i
  \end{split}\right\}
\]
Treated this way, the entailment of one WC-Datalog program, e.g. that in
\refeqn{continuous datalog: example WC-Datalog program},
may then be taken as a part or all of the data in a successive WC-Datalog
program, without modification of the existing semantics or loss of information
regarding the validity of the entailments.  However, in the interest
of being concise, such a successive WC-Datalog program may express valid
windows alongside any sliding windows. For example, 
a program composed of the set of rules $R^\prime \neq R$, the set of base facts
$F^{0\prime} \neq F^0$, a sliding window of range $x \geqslant r$ over the
in-order stream $S^\prime \neq S$, and the entailment of
...
\begin{gather}
  ...
  \labeleqn{continuous datalog: successive WC-Datalog program}
    \Pi^{C,x} = \langle R^\prime, F^{0\prime} \cup E^0, \{ W^{P \setminus N}, W^{S^\prime,x} \} \rangle \models \langle E^{0\prime}, P^\prime, N^\prime \rangle
\end{gather}

In conclusion, given a Continuous Datalog program $\Pi^C = \langle R,
F^0, S_0 \cup \dots \cup S_n \rangle$, we can apply to it a set of
windows of range $r_0,\dots,r_n$ to yield a Windowed Continuous Datalog
program $\Pi^{C,r} = \langle R, F^0, \{ W^{S_0, r_0}, \dots,
W^{S_n, r_n} \} \rangle$. We can evaluate these windows at
an instant $i \in \mathbb{T}$ to yield a Time-Annotated Datalog program
$\Pi^T = \langle R, F^0 \cup W^{S_0,r_0}_i \cup \dots \cup W^{S_n,r_n}_i
\rangle$, from we which can then strip the time annotations to yield a
conventional Datalog program $\Pi = \langle R, F \rangle$. The
relationship between these $\ast$-Datalog programs and their
