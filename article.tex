...
\begin{nestedsection}{R4 System: Rule-based Reasoning over RDF streams using Rete}{implementation}
  ...
  We have chosen RIF-Core to be the language in which rules can be represented, because it is compatible with RDF and other semantic web standards, as well as sharing the semantics of positive Datalog.
  ...
  We have also added a simple extension to RIF-Core (in line with the extension of Datalog described in \refsec{semantics}) that enables users to specify a window range as part of their rule set, as shown in \reffig{lst-S-RIF-Core}.
  ...
  \begin{nestedsection}{Continuous Reasoning with Rete}{implementation: continuous rete}
    The Rete algorithm \citep{forgy79}, which was introduced as a solution for the many pattern/many object matching problem, is a natural fit for our stream reasoning model because it is a dataflow approach that operates incrementally.
  ...
  \begin{nestedsection}{Continuous Reasoning in the Semantic Web}{implementation: continuous reasoning with RDF}
    ...
    In R4, an alpha node can receive streamed quads from any number of nodes through its single input, reflecting the multiple-source nature of CoDeR's streams.
    Each received quad is matched against some conditions, and is either dropped if there is no match, or propagated downstream to its successor node(s) in the event of a match.
    ...
    These windows are implemented as a combination of a priority queue (in which quads are ordered according to their time annotations) and a hashmap (in order to improve the speed of probing). Quads are removed from the priority queue when they expire (i.e. when their time annotations are less than the current time minus the window range); the corresponding entries in the hashmap are removed at the same time.

    When a join node is left-activated (i.e. it receives a token through its left input), it first adds the new token to its left window, then it prepares its right window by removing any expired items (as above) so that no outdated results are matched. The right window is then searched according to the join node conditions (using the hashmap). When a match is found, a new token is created by duplicating the left token and adding the right quad to the new token's quad list. The new token is annotated with the \emph{earliest} time with which its component quads were annotated, and then propagated to the next beta node, or to the terminal node if it is in the root node of a join tree. Conversely, when the join node is right-activated by receiving a quad from an alpha node, it adds the new quad to the right window, prepares the left window then matches the quad against the remaining tokens.
...
\begin{nestedsection}{Evaluation, Results and Discussion}{evaluation}
  ...
  \begin{nestedsection}{Methodological Considerations}
    Following the development of semantic stream processors, a number of benchmarks have emerged to test their performance: SRBench \citep{SRBench}, LSBench \citep{linkedstreamproc}, CSRBench and CityBench \citep{ali15}. While these benchmarks provide rich datasets, they mostly do not consider reasoning; with the exception of SRBench, none of the other benchmarks' queries require inference capabilities.

    One of the most important measures of any stream processing system is the maximum input throughput, defined in \citep{scharrenbach13} as the number of data elements in the input stream consumed by the system per time unit; we use this as the key performance indicator in our comparative evaluation of the three systems.
    In addition, it is necessary to confirm that the data produced by a system is complete and correct according to its semantics.
    It should be noted that, while correctness must be absolute, completeness may be expected to decline with decreased window size.
  \end{nestedsection}

  \begin{nestedsection}{Methodology}
   The experiments were performed on an Intel \mbox{Core i5} computer running at 3.2 GHz with 8 GB memory.  We set up Jtalis (the Java wrapper of Etalis) over SWI-Prolog v7.2.1 and installed Sparkwave v0.5.1.

We followed the experimentation strategy used by Sparkwave because it requires reasoning over background knowledge to correctly answer the query, using a synthetic dataset generated with the Berlin SPARQL Benchmark (BSBM) \citep{BSBMresults} containing 1.1 million triples (representing 100,000 limited-time offers made available by an online marketplace). For the background knowledge, we generated a small schema that described 329 product types arranged in a 4-level hierarchy.

    We configure each system with the generated schema, a rule inspired by a query from the BSBM and a window size (0.1, 1, 2, 5 and 10 second sliding windows are tested).
...
Forall ?offer ?product ?vendor ?price ?from ?to ?delivery ?webpage(
    ...
    We then measure the time taken for the system to consume all 1.1 million triples for each configuration of each system, from which we can calculate the maximum throughput and average latency of that system for each window size.
    We also compare the contents of the streams of entailments produced by each system against the set that is produced by statically reasoning over the conventional logic program composed of the same 1.1 million triple dataset and background knowledge;
    because of the tight localisation of offer IRIs in the stream, this provides a good benchmark of the correctness and completeness of each system for each window size.

  \end{nestedsection}

  \begin{nestedsection}{Comparative Results}{evaluation: comparative results}

    Firstly, for every window size, the streams produced by each system are correct, both in their contents with regard to the entailment of the conventional program, and their ordering with regard to the entailment semantics of WC-Datalog.
    Additionally, in all cases but that of Etalis with the smallest window size, the streams produced are complete with regard to the entailment of the conventional program.

    Secondly, we note that the rule presented in \reffig{BSBM-rule} takes significantly longer to process in Etalis (3 hours for the 1.1 million dataset with 1 second time window compared to ${{<}10}$ minutes for the other two systems).
    In order to further investigate this difference, we tried changing the \emph{And} into a series of \emph{seq}s \citep{EP-SPARQL}, which resulted in the system running 20x times faster (7.5 minutes for the same test).
    Despite being semantically different, we would not expect such a drastic difference in running time unless Etalis were heavily optimised for the \emph{seq} operator;
    this is likely, as Etalis is intended for \emph{event processing} rather than continuous querying/reasoning, making the order of arrival of triples more relevant than their simple coincidence in the system.

    In order to present graphics that provide a meaningful comparison between the time efficiency of each system with regards to changing window ranges, as in \reffig{all-systems-varying-windows}, we chose to eliminate the results for Etalis using \emph{And} from our data set, but include those using the rule modified to use \emph{seq} when evaluating Etalis.
    It should be noted that we recognise that this modified rule is not semantically equivalent to that by which we evaluate R4 and Sparkwave, which cannot express the \emph{seq} operator, but is sufficient to contrast the effect of window range on the time-efficiency of the three systems.
    \begin{figure}
      \centering
      \includegraphics[width=0.45\textwidth]{all-systems-varying-windows-throughput}\\
      \includegraphics[width=0.45\textwidth]{all-systems-varying-windows-latency}
      \caption{A comparison of the maximum throughputs and average processing latencies of the systems R4, Sparkwave and Etalis, given windows of various ranges over the input stream.}
      \labelfig{all-systems-varying-windows}
    \end{figure}

    As shown in \reffig{all-systems-varying-windows}, R4 is significantly faster than both Etalis and Sparkwave for the tested window sizes.
    Interestingly, while the maximum throughput decreases and average processing latency increases (apparently asymptotically) with window range similarly in both Sparkwave and R4, those of Etalis using \emph{seq} appear to remain approximately constant for all window ranges tested.
    It should be noted that this behaviour of Etalis remains in the case where the query uses \emph{And}.
    However, despite its constancy, the throughput and processing latency of Etalis are so dramatically lower/higher than the other systems, respectively, that we project that R4 will remain faster than both Sparkwave and Etalis in cases where the window sizes are orders of magnitude larger than those tested.
